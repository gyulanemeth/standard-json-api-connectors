...
describe('patch', () => {
  ...
  test('with timeout', async () => {
    ...
    const patch = createPatchConnector(fetch, 'https://test.com', params => `/v1/something/${params.somethingId}/else/`, params => ({ Authorization: 'Bearer test-token' }), { timeout: 100 })

    await expect(patch({ somethingId: 3 }, { example: 'body' })).rejects.toThrow('Failed to fetch: CORS error. Please contact support')
  })
  ...
  test('success with timeout', async () => {
    ...
    const patch = createPatchConnector(fetch, 'https://test.com', params => `/v1/something/${params.somethingId}/else/`, params => ({ Authorization: 'Bearer test-token' }), { timeout: 4000 })
    const response = await patch({ somethingId: 3 }, { example: 'body' })
    expect(fetch.mock.lastCall).toEqual([
      'https://test.com/v1/something/3/else/',
      {
